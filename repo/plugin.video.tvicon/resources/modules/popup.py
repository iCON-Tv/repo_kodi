...
FANART         = os.path.join(ADDONPATH,   'fundo.jpg')
...
def autoConfigQ():
	class FTGac(xbmcgui.WindowXMLDialog):
		def __init__(self,*args,**kwargs):
			self.title   ='[COLOR %s]Configurações Avançadas[/COLOR]' % (COLOR2)
			...
			current1 = '[COLOR %s]Video Cache Tamanho[/COLOR]=[COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.currentVideo)
			recommended1 = '[COLOR %s]Video Cache Tamanho:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.recommendedVideo)
			...
			self.msgbox='[COLOR %s]Estas são as melhores configurações atualmente para o seu dispositivo.\n\nEscolha escrever e eles serão gravados na advancesettings.xml[/COLOR]\r\n\r\n%s\r\n%s\r\n%s\r\n%s' %(COLOR1, recommended4, recommended1, recommended3, recommended2)
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Atualmente existe um ativo [COLOR %s]AdvancedSettings.xml[/COLOR], você gostaria de removê-lo e continuar?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Remove Settings[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Write[/COLOR][/B]")
				if choice == 0: return
			...
			if KODIV < 17:
				...
				with open(ADVANCED, 'w+') as f:
					...
					f.write('</advancedsettings>\n')
				f.close()
				tools.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]AdvancedSettings.xml foram re-escritos[/COLOR]' % COLOR2)
			self.CloseWindow()
