...
fanart			  = os.path.join(PLUGIN,  'fundo.jpg')
...
#############################################################################
adult_tags = ['xxx','xXx','XXX','adult','Adult','ADULT','adults','Adults','ADULTS','adult','Adulto','ADULTO','adultos','Adultos','ADULTOS''porn','Porn','PORN']
...
def start(signin):
	if username == "":
		...
		if login_data == 0:
			...
			line3 = "" 
			xbmcgui.Dialog().ok('Atençãon', line1+'\n'+line2+'\n'+line3)
			start()
		else:
			line1 = "Login feito com Sucesso"
...
def home():
	...
	tools.addDir('Ao Vivo','live',1,iconlive,background,'')
	tools.addDir('Séries','live',18,iconTvseries,background,'')
	if xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)'):
		tools.addDir('Guia de Tv','pvr',7,icontvguide,background,'')
...
def Livelist(url):
	...
	for topic in doc.getElementsByTagName('channel'):
		...
		try:
			...
			thumb = live
			desc = 'Nenhuma informação disponível'
		if xbmcaddon.Addon().getSetting('hidexxx')=='false':
...
def searchdialog():
	search = control.inputDialog(heading='Pesquisa '+ADDON_NAME+':')
	if search=="":
		return
	else:
		return search
...
def settingsmenu():
	...
	tools.addDir('Editar configurações avançadas','ADS',10,icon,background,'')
	tools.addDir('META está %s'%META,'META',10,icon,background,META)
	tools.addDir('Ocultar conteúdo adulto %s'%xxx,'XXX',10,icon,background,xxx)
...
def addonsettings(url,description):
	...
	if	 url =="clearcache":
		...
		dialog = xbmcgui.Dialog().yesno(ADDON_NAME,'Você gostaria de configurar o guia de TV?')
		if dialog:
			pvrsetup()
			xbmcgui.Dialog().ok(ADDON_NAME, 'Integração PVR concluída, reinicie o Kodi para que as alterações entrem em vigor')
	elif url =="Itv":
		...
		if 'ON' in description:
			pas = tools.keypopup('Digite a senha de adulto:')
...
def adult_set():
	dialog = DIALOG.yesno(ADDON_NAME,'Deseja ocultar o Menu Adulto? \nVocê pode alterar isso nas configurações mais tarde.')
	if dialog:
		...
		pass
	dialog = DIALOG.yesno(ADDON_NAME,'Você gostaria de proteger com senha o conteúdo adulto? \nVocê pode alterar isso nas configurações mais tarde.')
	if dialog:
		control.setSetting('xxx_pwset','true')
		adultpw = tools.keypopup('Digite a Senha')
...
def correctPVR():
	...
	if choice:
		...
		DP.create(ADDON_NAME, "Por favor Aguarde")
		tools.gen_m3u(panel_api, M3U_PATH)
		m3u_do = 'yes'
	try:
		...
		xbmc.executebuiltin("Container.Refresh")
		DIALOG.ok(ADDON_NAME,"PVR Client atualizado, o Kodi precisa ser reiniciado para que as alterações entrem em vigor, clique em ok para sair do kodi e, em seguida, reinicie")
		os._exit(1)
	except:
		DIALOG.ok(ADDON_NAME,"PVR Client: Erro desconhecido ou PVR já configurado")

def tvguidesetup():
		dialog = DIALOG.yesno(ADDON_NAME,'Gostaria que o '+ADDON_NAME+' configurasse o guia de TV para você?')
		if dialog:
			pvrsetup()
			DIALOG.ok(ADDON_NAME, 'Tudo Pronto! \n Reinicie o Kodi para que as alterações entrem em vigor')
...
def extras():
	tools.addDir('Iniciar Speed Test','ST',10,icon,background,'')
	try:
		if xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)'):
			tools.addDir('Config (EPG) PVR Guide','tv',10,icon,background,'')
		if not xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)'):
			tools.addDir('Instalar (EPG) PVR Guide','Itv',10,icon,background,'')
		if os.path.exists(M3U_PATH):
			tools.addDir('Reiniciar M3U','RefM3U',10,icon,background,'')
	except:pass
